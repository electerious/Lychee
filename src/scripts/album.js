...
album.add = function () {
	...
	const action = function (data) {
		...
		let SubAlbumYes = data.SubYes
		let SubAlbumCreate = false
		const isNumber = (n) => (!isNaN(parseFloat(n)) && isFinite(n))

		basicModal.close()
		if (SubAlbumYes) SubAlbumCreate = true
...
album.addSub = function () {
	...
	const action = function (data) {
		...
		api.post('Album::get', params, function (data) {
			...
			parentComma += ',' + data.parent



			api.post('Album::add', {
				title, parent: parentComma
			}, function (data) {
	...
	api.post('Album::getAll', {
		...
		for (Album in data.albums) {
			let title = data.albums[Album].title

			cmbxOptions += `<option value='` + data.albums[Album].id + `'>` + title + `</option>`

		}
		cmbxOptions += '</select>'

		basicModal.show({
			body: `<p>Enter a title for the new album: <input class='text' name='title' type='text' maxlength='50' placeholder='Title' value='Untitled'></p><p>` + cmbxOptions + `</p>`,
			buttons: {
				action: {
					title: 'Create Album',
					fn: action
...
album.setPublic = function (albumID, modal, e) {
	...
	api.post('Album::getAll', params, function (data) {
		...
		data.albums.forEach(function (alb) {
			...
			if (parents.indexOf(albumID) > -1 || alb.id == albumID) {
				params = {
					albumID: alb.id,
...
album.setParent = function (albumIDs, parent) {
	if (!albumIDs) return false
	if (albumIDs instanceof Array === false) albumIDs = [albumIDs]
	albumIDs.forEach(function (alb) {
		let params = {
			albumID: alb,
			parent: parseInt(parent)
		}
		api.post('Album::setParent', params, function (data) {
			if (data !== true && data !== "true") lychee.error(null, params, data)
