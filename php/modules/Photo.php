...
class Photo extends Module {
	...
	public function setTitle($title) {
		...
		if (!$result) {
			...
			return false;
				...
				$query	= Database::prepare($this->database, "SELECT url FROM ? WHERE id = '?' LIMIT 1", array(LYCHEE_TABLE_PHOTOS, $this->photoIDs));
		$photos	= $this->database->query($query);
		$photo	= $photos->fetch_object();
		$objIPTC = new IPTC(LYCHEE_UPLOADS_BIG . $photo->url);

		//set keywords
		$objIPTC->setValue(IPTC_HEADLINE, $title);
		return true;
	...
	public function setDescription($description) {
		...
		//set keywords
		$objIPTC->setValue(IPTC_CAPTION, $description);
		return true;
	...
	public function setTags($tags) {
		...
		//set keywords
		$objIPTC->setValue(IPTC_KEYWORDS, $tags);
